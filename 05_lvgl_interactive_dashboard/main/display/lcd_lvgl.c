...
#define LCD_BACKLIGHT_MAX_DUTY ((1U << LEDC_TIMER_12_BIT) - 1U)
...
esp_err_t lcd_backlight_set_brightness(uint8_t percent)
{
    ...
    uint32_t duty = LCD_BACKLIGHT_MAX_DUTY * percent / 100U;
    ESP_RETURN_ON_ERROR(ledc_set_duty(LEDC_LOW_SPEED_MODE, LCD_BACKLIGHT_CHANNEL, duty),
                        "display", "backlight duty failed");
...
esp_err_t lcd_lvgl_init(lv_disp_t **display)
{
    ...
    ESP_RETURN_ON_ERROR(ledc_timer_config(&backlight_timer), "display",
    ...
    const ledc_channel_config_t backlight_channel = {
        ...
        .duty = LCD_BACKLIGHT_MAX_DUTY,
        .hpoint = 0,
    };
    ESP_RETURN_ON_ERROR(ledc_channel_config(&backlight_channel), "display",
